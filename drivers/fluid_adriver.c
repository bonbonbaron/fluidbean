...
static U8 fluid_adriver_disable_mask[(G_N_ELEMENTS(fluid_audio_drivers) + 7) / 8] = {0};
...
{
    unsigned int i;
    uint8_t      disable_mask[G_N_ELEMENTS(fluid_adriver_disable_mask)];
    ...
    for(i = 0; adrivers[i] != NULL; i++)
    {
        ...
        for(j = 0; j < G_N_ELEMENTS(fluid_audio_drivers) - 1; j++)
        {
        ...
        if(j >= G_N_ELEMENTS(fluid_audio_drivers) - 1)
        {
