...
getNumOutputs(AudioDeviceID deviceID)
{
    ...
    {
        AudioBufferList *bufList = MALLOC(size);
...
setChannelMap(AudioUnit outputUnit, int audioChannels, const char *mapString)
{
    ...
    if(status != noErr)
    {
        LOG(ERR, "Failed to get the channel map size. Status=%ld\n", (long int) status);
        return;
    }
    ...
    channelMap = ARRAY(int, numberOfChannels);
    if(channelMap == NULL)
    {
        LOG(ERR, "Out of memory.\n");
        return;
    ...
    MEMSET(channelMap, 0xff, propertySize);
    ...
    if(status != noErr)
    {
        LOG(ERR, "Failed to get the existing channel map. Status=%ld\n", (long int) status);
        FREE(channelMap);
        return;
    }
    ...
    for(i = 0; i < numberOfChannels; i++)
    {
        ...
        {
            LOG(DBG, "Channel map of output channel %d is out-of-range. Silencing.", i);
            channelMap[i] = -1;
        }
    }
    ...
    if(status != noErr)
    {
        LOG(ERR, "Failed to set the channel map. Status=%ld\n", (long int) status);
    }
...
void
coreAudioDriverSettings(Settings *settings)
{
    int i;
    UInt32 size;
    AudioObjectPropertyAddress pa;
 ...
 */
audioDriverT *
newCoreAudioDriver(Settings *settings, Synthesizer *synth)
{
    return newCoreAudioDriver2(settings,
                                        NULL,
                                        synth);
}
 ...
 */
audioDriverT *
newCoreAudioDriver2(Settings *settings, audioFuncT func, void *data)
{
    ...
    MEMSET(dev, 0, sizeof(coreAudioDriverT));
    ...
    if(comp == NULL)
    {
        LOG(ERR, "Failed to get the default audio device");
        goto errorRecovery;
    }
    ...
    if(status != noErr)
    {
        LOG(ERR, "Failed to open the default audio device. Status=%ld\n", (long int)status);
        goto errorRecovery;
    }
    ...
    if(status != noErr)
    {
        LOG(ERR, "Error setting the audio callback. Status=%ld\n", (long int)status);
        goto errorRecovery;
    }
    ...
    if(settingsDupstr(settings, "audio.coreaudio.device", &devname) == OK   /* alloc device name */
        ...
        if(OK(AudioObjectGetPropertyDataSize(kAudioObjectSystemObject, &pa, 0, 0, &size)))
        {
            ...
            if(OK(AudioObjectGetPropertyData(kAudioObjectSystemObject, &pa, 0, 0, &size, devs)))
            {
                for(i = 0; i < num; ++i)
                {
                    ...
                    if(OK(AudioObjectGetPropertyData(devs[i], &pa, 0, 0, &size, name)))
                    {
                        if(getNumOutputs(devs[i]) > 0 && STRCASECMP(devname, name) == 0)
                        {
                            ...
                            if(status != noErr)
                            {
                                LOG(ERR, "Error setting the selected output device. Status=%ld\n", (long int)status);
                                goto errorRecovery;
                            }
                        }
                    }
    ...
    LOG(DBG, "mSampleRate %g", dev->format.mSampleRate);
    LOG(DBG, "mFormatFlags %08X", dev->format.mFormatFlags);
    LOG(DBG, "mBytesPerPacket %d", dev->format.mBytesPerPacket);
    LOG(DBG, "mFramesPerPacket %d", dev->format.mFramesPerPacket);
    LOG(DBG, "mChannelsPerFrame %d", dev->format.mChannelsPerFrame);
    LOG(DBG, "mBytesPerFrame %d", dev->format.mBytesPerFrame);
    LOG(DBG, "mBitsPerChannel %d", dev->format.mBitsPerChannel);
    ...
    if(status != noErr)
    {
        LOG(ERR, "Error setting the audio format. Status=%ld\n", (long int)status);
        goto errorRecovery;
    }

    if(settingsDupstr(settings, "audio.coreaudio.channel-map", &channelMap) == OK   /* alloc channel map */
            && channelMap && strlen(channelMap) > 0)
    {
        setChannelMap(dev->outputUnit, audioChannels, channelMap);
    }
    FREE(channelMap);  /* free channel map */
    ...
    if(status != noErr)
    {
        LOG(ERR, "Failed to set the MaximumFramesPerSlice. Status=%ld\n", (long int)status);
        goto errorRecovery;
    }

    LOG(DBG, "MaximumFramesPerSlice = %d", dev->bufferSize);

    dev->buffers = ARRAY(float *, audioChannels);
    ...
    if(status != noErr)
    {
        LOG(ERR, "Error calling AudioUnitInitialize(). Status=%ld\n", (long int)status);
        goto errorRecovery;
    }
    ...
    if(status != noErr)
    {
        LOG(ERR, "Error calling AudioOutputUnitStart(). Status=%ld\n", (long int)status);
        goto errorRecovery;
    }
...
errorRecovery:
    ...
    deleteCoreAudioDriver((audioDriverT *) dev);
    return NULL;
}

/*
 * deleteCoreAudioDriver
 */
void
deleteCoreAudioDriver(audioDriverT *p)
{
                          ...
                          AudioBufferList *ioData)
{
    ...
    for(i = 0; i < ioData->mNumberBuffers && i < dev->bufferCount; i++)
    {
        ...
        MEMSET(dev->buffers[i], 0, len * sizeof(float));
    }
