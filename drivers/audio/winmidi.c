...
static char *winmidiGetDeviceName(int devIdx, char *devName)
{
    ...
    newDevName = MALLOC(size + 2 + STRLEN(devName));

    if(newDevName)
    {
        /* the name is filled if allocation is successful */
        SPRINTF(newDevName, "%d:%s", devIdx, devName);
    }
    else
    {
...
static DWORD WINAPI winmidiAddSysexThread(void *data)
{
    ...
    for(;;)
    {
        ...
        if(code < 0)
        {
            LOG(ERR, "winmidiAddSysexThread: GetMessage() failed.");
            break;
        }
...
winmidiParseDeviceName(winmidiDriverT *dev, char *devName)
{
    ...
    */
    STRCPY(cpyDevName, devName); /* strtok() will overwrite */
    nextIdx = cpyDevName;
    ...
    while(NULL != (curIdx = strtok(&nextIdx, " ;")))
    {
        ...
        char *endIdx = curIdx;
        devIdx = STRTOL(curIdx, &endIdx, 10);
    ...
    if(!devCount)
    {
        ...
        if(STRCASECMP("default", devName) != 0)
        {
            ...
            for(i = 0; i < num; i++)
            {
...
#else
                    strCmpRes = STRCASECMP(devName, newDevName);
#endif

                    LOG(DBG, "Testing midi device \"%s\"", newDevName);
                    FREE(newDevName);

                    if(strCmpRes == 0)
                    {
                        LOG(DBG, "Selected midi device number: %u", i);
                        devIdx = i;
                        devCount = 1;
                        break;
                    }
                ...
                {
                    LOG(DBG, "Error testing midi device %u of %u: %s (error %d)",
    ...
    if(num < devCount)
    {
        LOG(ERR, "not enough MIDI in devices found. Expected:%d found:%d",
                  devCount, num);
        devCount = 0;
    }
 ...
 */
midiDriverT *
newWinmidiDriver(Settings *settings,
                         handleMidiEventFuncT handler, void *data)
{
    ...
    if(settingsCopystr(settings, "midi.winmidi.device", devName, MAXPNAMELEN) != OK)
    {
        LOG(DBG, "No MIDI in device selected, using \"default\"");
    ...
    if(!maxDevices)
    {
        LOG(ERR, "Device \"%s\" does not exists", devName);
        return NULL;
    }
    ...
    MEMSET(dev, 0, i); /* reset structure members */
    ...
    for(i = 0; i < dev->devCount; i++)
    {
        ...
        LOG(DBG, "opening device at index %d", devInfos->devIdx);
        ...
        if(res != MMSYSERR_NOERROR)
        {
            LOG(ERR, "Couldn't open MIDI input: %s (error %d)",
        ...
        for(j = 0; j < MIDI_SYSEX_BUF_COUNT; j++)
        {
            ...
            if(res == MMSYSERR_NOERROR)
            {
                ...
                if(res != MMSYSERR_NOERROR)
                {
                    LOG(WARN, "Failed to prepare MIDI SYSEX buffer: %s (error %d)",
                              winmidiInputError(strError, res), res);
    ...
    if(dev->hThread == NULL)
    {
        LOG(ERR, "Failed to create SYSEX buffer processing thread");
        goto errorRecovery;
    }
    ...
    for(i = 0; i < dev->devCount; i++)
    {
        ...
        {
            LOG(ERR, "Failed to start the MIDI input. MIDI input not available.");
            goto errorRecovery;
        }
    }
...
errorRecovery:
    ...
    deleteWinmidiDriver((midiDriverT *) dev);
    return NULL;
}
 ...
 */
void
deleteWinmidiDriver(midiDriverT *p)
{
    int i, j;
