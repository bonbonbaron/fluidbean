...
audioDriverT *
newDartAudioDriver(Settings *settings, Synthesizer *synth)
{
    ...
    MEMSET(dev, 0, sizeof(dartAudioDriverT));
    ...
    if(mHmodMDM == NULLHANDLE)
    {
        ...
        if(rc != 0)
        {
            LOG(ERR, "Cannot load MDM.DLL for DART due to %s", szFailedName);
            goto errorRecovery;
        }
        ...
        if(rc != 0)
        {
            LOG(ERR, "Cannot find mciSendCommand() in MDM.DLL");
    ...
    {
        LOG(ERR, "Cannot open DART, rc = %lu", rc);
        goto errorRecovery;
    }
    ...
    if(rc != MCIERR_SUCCESS)
    {
        LOG(ERR, "Cannot setup DART, rc = %lu", rc);
        goto errorRecovery;
    }
    ...
    {
        LOG(ERR, "Cannot allocate memory for DART, rc = %lu", rc);
        goto errorRecovery;
    }
    ...
    {
        MEMSET(dev->MixBuffers[i].pBuffer, 0, dev->BufferParms.ulBufferSize);
...
errorRecovery:
    ...
    deleteDartAudioDriver((audioDriverT *) dev);
    return NULL;
}
...
static LONG APIENTRY dartAudioRun(ULONG ulStatus, PMCI_MIX_BUFFER pBuffer, ULONG ulFlags)
{
    ...
    case MIX_WRITE_COMPLETE:                    /* for playback  */
        MEMSET(pBuffer->pBuffer, 0, pBuffer->ulBufferLength);
