 ...
 */
int
startSndmgrAudioDriver(Settings *settings,
                                ...
                                int bufferSize)
{
    ...
    /* the channel */
    LOG(DBG, "-SndManager@2");
    err = SndNewChannel(&channel, sampledSynth, initStereo, NULL);

    if((err != noErr) || (channel == NULL))
    {
        LOG(ERR, "Failed to allocate a sound channel (error %i)", err);
        return err;
    }

    /* the double buffer struct */
    LOG(DBG, "-SndManager@3");
    doubleHeader = NEW(SndDoubleBufferHeader2);
    ...
    /* the 2 doublebuffers */
    LOG(DBG, "-SndManager@5");

    for(i = 0; i < 2; i++)
    {
        doubleBuffer = (SndDoubleBufferPtr) MALLOC(sizeof(SndDoubleBuffer)
                       + dev->bufferByteSize);
    ...
    /* start */
    LOG(DBG, "-SndManager@6");

    err = SndPlayDoubleBuffer(channel, (SndDoubleBufferHeader *)doubleHeader);

    if(err != noErr)
    {
        LOG(ERR, "Failed to start the sound driver (error %i)", err);
        return err;
    }

    LOG(DBG, "-SndManager@7");
    return 0;
}

/*
 * newSndmgrAudioDriver
 * This implementation used the 16bit format.
 */
audioDriverT *
newSndmgrAudioDriver(Settings *settings, Synthesizer *synth)
{
    ...
    MEMSET(dev, 0, sizeof(sndmgrAudioDriverT));

    dev->callbackIsAudioFunc = false;
    dev->data = (void *)synth;
    dev->callback = NULL;

    if(startSndmgrAudioDriver(settings, dev, bufferSize) != 0)
    {
        deleteSndmgrAudioDriver((audioDriverT *)dev);
        return NULL;
    }
...
newSndmgrAudioDriver2(Settings *settings, audioFuncT func, void *data)
{
    ...
    dev->convbuffers[0] = ARRAY(float, bufferSize);
    dev->convbuffers[1] = ARRAY(float, bufferSize);
...
void deleteSndmgrAudioDriver(audioDriverT *p)
{
    ...
    {
        FREE(dev->doubleHeader->dbhBufferPtr[0]);
        FREE(dev->doubleHeader->dbhBufferPtr[1]);
        FREE(dev->doubleHeader);
    }

    FREE(dev->convbuffers[0]);
    FREE(dev->convbuffers[1]);
    FREE(dev);
}
...
void  pascal sndmgrCallback(SndChannelPtr chan, SndDoubleBufferPtr doubleBuffer)
{
    ...
    else
    {
        ...
        synthWriteS16((Synthesizer *)dev->data, bufferSize, buf, 0, 2, buf, 1, 2);
    }
