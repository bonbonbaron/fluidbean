 ...
 * 03/06/03 : Adapdation for Synth API
 * 18/03/04 : In application mode, connect MidiShare to the synth client (midishareOpenAppl)
 */
...
#define MSHSlotName		"\psynth"
#define MSHDriverName	"\psynth"
#else
#define MSHSlotName		"synth"
#define MSHDriverName	"synth"
#endif
...
#endif
    SlotRefNum	slotRef;
    unsigned char sysexbuf[MIDI_PARSER_MAX_DATA_SIZE];
 ...
 */
midiDriverT *
newMidishareMidiDriver(Settings *settings,
                                ...
                                void *data)
{
    ...
    LOG(WARN,
              ...
              "mailing list at -dev@nongnu.org - otherwise this driver might be removed\n"
              "in a future release of Synth!\n"
              "================ MidiShare MIDI driver has been deprecated! =================\n"
              "\n"
    );
    ...
    MEMSET(dev, 0, sizeof(midishareMidiDriverT));
    ...
    if(dev->filter == 0)
    {
        LOG(ERR, "Can not allocate MidiShare filter");
        goto errorRecovery;
    }
...
errorRecovery:
    deleteMidishareMidiDriver((midiDriverT *) dev);
    return NULL;
}
...
static void midishareMidiDriverReceive(short ref)
{
    ...
    while((e = MidiGetEv(ref)))
    {
        ...
        case typeSysEx:
            ...
            if(count == 0 || count > MIDI_PARSER_MAX_DATA_SIZE)
            {
                MidiFreeEv(e);
                continue;
            }
...
static int midishareOpenDriver(midishareMidiDriverT *dev)
{
    ...
    if(dev->refnum < 0)
    {
        LOG(ERR, "Can not open MidiShare Application client");
        return 0;
    }
...
static int midishareOpenAppl(midishareMidiDriverT *dev)
{
    ...
    if(dev->refnum < 0)
    {
        LOG(ERR, "Can not open MidiShare Driver client");
        return 0;
    }
