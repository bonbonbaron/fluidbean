...
static int dsoundWriteProcessedChannels(Synthesizer *data, int len,
                               int channelsCount,
...
typedef struct
{
    ...
    DWORD threadID;
    void *synth; /* synth instance, or user pointer if custom processing function is defined */
    audioFuncT func;
...
*/
audioDriverT *
newDsoundAudioDriver(Settings *settings, Synthesizer *synth)
{
    return newDsoundAudioDriver2(settings, NULL, synth);
}

audioDriverT *
newDsoundAudioDriver2(Settings *settings, audioFuncT func, void *data)
{
    HRESULT hr;
    DSBUFFERDESC desc;
    ...
    MEMSET(dev, 0, sizeof(dsoundAudioDriverT));
    dev->synth = data;
    dev->func = func;
    ...
    else
    {
        ...
        {
            dev->drybuf[i] = ARRAY(float, periods * periodSize);
    ...
    if(settingsDupstr(settings, "audio.dsound.device", &devsel.devname) == OK /* ++ alloc device name */
    ...
    if(devsel.devname)
    {
        FREE(devsel.devname);    /* -- free device name */
    }
    ...
    if(devsel.devGUID)
    {
        FREE(devsel.devGUID);    /* -- free device GUID */
    }

    if(hr != DS_OK)
    {
        LOG(ERR, "Failed to create the DirectSound object: '%s'", win32_error(hr));
        goto errorRecovery;
    }
    ...
    if(hr != DS_OK)
    {
        LOG(ERR, "Failed to set the cooperative level: '%s'", win32_error(hr));
        goto errorRecovery;
    }
    ...
    if(hr != DS_OK)
    {
        LOG(ERR, "Failed to query the device capacities: '%s'", win32_error(hr));
        goto errorRecovery;
    }
    ...
    if(hr != DS_OK)
    {
        LOG(ERR, "Failed to allocate the primary buffer: '%s'", win32_error(hr));
        goto errorRecovery;
    }
    ...
    if(hr != DS_OK)
    {
        LOG(WARN, "Can't set format of primary sound buffer: '%s'", win32_error(hr));
    }
    ...
    */
    ...
    if(hr != DS_OK)
    {
        LOG(ERR, "dsound: Can't create sound buffer: '%s'", win32_error(hr));
        goto errorRecovery;
    }
    ...
    {
        LOG(PANIC, "Failed to lock the audio buffer: '%s'", win32_error(hr));
        goto errorRecovery;
    }
    ...
    if(dev->thread == NULL)
    {
        LOG(ERR, "Failed to create DSound audio thread: '%s'", getWindowsError());
        goto errorRecovery;
    }
...
errorRecovery:
    deleteDsoundAudioDriver((audioDriverT *) dev);
    return NULL;
}


void deleteDsoundAudioDriver(audioDriverT *d)
{
    int i;
    ...
    */
    ...
    if(dev->thread != NULL)
    {
        ...
        if(WaitForSingleObject(dev->thread, 2000) != WAIT_OBJECT_0)
        {
            ...
            LOG(DBG, "Couldn't join the audio thread. killing it.");
    ...
    */
    ...
    for(;;)
    {
        ...
        if(bytes >= dev->bufferByteSize)
        {
            ...
            {
                LOG(PANIC, "Failed to lock the audio buffer. System lockup might follow. Exiting.");
                ExitProcess((UINT)-1);
            }
            ...
            if(bytes1 > 0)
            {
                ...
                */
                ...
                {
                    dev->write((Synthesizer*)dev, frames, dev->channelsCount,
                            channelsOut, channelsOff, channelsIncr);
                }
