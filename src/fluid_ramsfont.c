...
/* Prototypes */
int fluid_rampreset_add_sample (fluid_rampreset_t * preset,
																fluid_sample_t * sample, int lokey,
																int hikey);
int fluid_rampreset_izone_set_gen (fluid_rampreset_t * preset,
																	 fluid_sample_t * sample, int gen_type,
																	 float value);
int fluid_rampreset_izone_set_loop (fluid_rampreset_t * preset,
																		fluid_sample_t * sample, int on,
																		float loopstart, float loopend);
int fluid_rampreset_remove_izone (fluid_rampreset_t * preset,
																	fluid_sample_t * sample);
void fluid_rampreset_updatevoices (fluid_rampreset_t * preset, int gen_type,
																	 float val);
...
fluid_preset_t *fluid_ramsfont_sfont_get_preset (fluid_sfont_t * sfont,
	...
	rampreset =
		fluid_ramsfont_get_preset ((fluid_ramsfont_t *) sfont->data, bank,
															 prenum);
...
int
fluid_rampreset_preset_noteon (fluid_preset_t * preset, fluid_synth_t * synth,
															 int chan, int key, int vel) {
	return fluid_rampreset_noteon ((fluid_rampreset_t *) preset->data, synth,
																 chan, key, vel);
}
 ...
 */
int fluid_ramsfont_set_name (fluid_ramsfont_t * sfont, char *name) {
 ...
 */
int
fluid_ramsfont_add_preset (fluid_ramsfont_t * sfont,
													 fluid_rampreset_t * preset) {
 ...
 */
int
fluid_ramsfont_add_izone (fluid_ramsfont_t * sfont,
													unsigned int bank, unsigned int num,
													fluid_sample_t * sample, int lokey, int hikey) {
...
int
fluid_ramsfont_remove_izone (fluid_ramsfont_t * sfont,
														 unsigned int bank, unsigned int num,
														 fluid_sample_t * sample) {
	int err;
	fluid_rampreset_t *preset = fluid_ramsfont_get_preset (sfont, bank, num);
...
int
fluid_ramsfont_izone_set_gen (fluid_ramsfont_t * sfont,
															unsigned int bank, unsigned int num,
															fluid_sample_t * sample, int gen_type,
															float value) {
...
int
fluid_ramsfont_izone_set_loop (fluid_ramsfont_t * sfont,
															 unsigned int bank, unsigned int num,
															 fluid_sample_t * sample, int on,
															 float loopstart, float loopend) {
	fluid_rampreset_t *preset = fluid_ramsfont_get_preset (sfont, bank, num);
	...
	return fluid_rampreset_izone_set_loop (preset, sample, on, loopstart,
																				 loopend);
}
 ...
 */
fluid_rampreset_t *fluid_ramsfont_get_preset (fluid_ramsfont_t * sfont,
																							unsigned int bank,
																							unsigned int num) {
 ...
 */
int
fluid_rampreset_add_zone (fluid_rampreset_t * preset,
													fluid_preset_zone_t * zone) {
	if (preset->zone == NULL) {
		zone->next = NULL;
		preset->zone = zone;
 ...
 */
int
fluid_rampreset_add_sample (fluid_rampreset_t * preset,
														fluid_sample_t * sample, int lokey, int hikey) {
...
fluid_inst_zone_t *fluid_rampreset_izoneforsample (fluid_rampreset_t * preset,
																									 fluid_sample_t * sample) {
	fluid_inst_t *inst;
...
int
fluid_rampreset_izone_set_loop (fluid_rampreset_t * preset,
																fluid_sample_t * sample, int on,
																float loopstart, float loopend) {
	...
	fluid_rampreset_updatevoices (preset, GEN_SAMPLEMODE,
...
int
fluid_rampreset_izone_set_gen (fluid_rampreset_t * preset,
															 fluid_sample_t * sample, int gen_type,
															 float value) {
	fluid_inst_zone_t *izone = fluid_rampreset_izoneforsample (preset, sample);
	if (izone == NULL)
...
int
fluid_rampreset_remove_izone (fluid_rampreset_t * preset,
															fluid_sample_t * sample) {
	fluid_inst_t *inst;
	...
	// the sample can be cleared up
	{
		...
		while (tmp) {
			fluid_rampreset_voice_t *presetvoice =
				(fluid_rampreset_voice_t *) (tmp->data);
			fluid_voice_t *voice = presetvoice->voice;
			if (fluid_voice_is_playing (voice)
					&& (fluid_voice_get_id (voice) == presetvoice->voiceID)) {
 ...
 */
int
fluid_rampreset_remembervoice (fluid_rampreset_t * preset,
															 fluid_voice_t * voice) {
	...
	preset->presetvoices =
		fluid_list_append (preset->presetvoices, (void *) presetvoice);
	if (preset->presetvoices == NULL) {
 ...
 */
void
fluid_rampreset_updatevoices (fluid_rampreset_t * preset, int gen_type,
															float val) {
 ...
 */
int
fluid_rampreset_noteon (fluid_rampreset_t * preset, fluid_synth_t * synth,
												int chan, int key, int vel) {
 ...
 */
int fluid_sample_set_name (fluid_sample_t * sample, char *name) {
...
fluid_sample_set_sound_data (fluid_sample_t * sample, short *data,
														 unsigned int nbframes, short copy_data,
														 int rootkey) {
	...
	if (copy_data) {
		...
		FLUID_MEMSET (sample->data, 0,
									storedNbFrames * 2 + 4 * SAMPLE_LOOP_MARGIN);
		FLUID_MEMCPY ((char *) (sample->data) + 2 * SAMPLE_LOOP_MARGIN, data,
									nbframes * 2);

#if 0
		/* this would do the fill of the margins */
		FLUID_MEMCPY ((char *) (sample->data) + 2 * SAMPLE_LOOP_MARGIN +
									storedNbFrames * 2, (char *) data, 2 * SAMPLE_LOOP_MARGIN);
		FLUID_MEMCPY ((char *) (sample->data),
									(char *) data + nbframes * 2 - 2 * SAMPLE_LOOP_MARGIN,
									2 * SAMPLE_LOOP_MARGIN);
#endif
 ...
 */
int delete_fluid_ramsample (fluid_sample_t * sample) {
