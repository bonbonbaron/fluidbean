 ...
 */
int
fluid_synth_modulate_voices (fluid_synth_t * synth, int chan, int is_cc,
														 int ctrl) {
	int i;
	fluid_voice_t *voice;
 ...
 */
int
fluid_synth_bank_select (fluid_synth_t * synth, int chan, U32 bank) {
	if ((chan >= 0) && (chan < synth->midi_channels)) {
 ...
 */
int
fluid_synth_sfont_select (fluid_synth_t * synth, int chan,
													U32 sfont_id) {
	if ((chan >= 0) && (chan < synth->midi_channels)) {
...
int
fluid_synth_process (fluid_synth_t * synth, int len,
 ...
 */
void
fluid_synth_kill_by_exclusive_class (fluid_synth_t * synth,
...
int fluid_synth_sfload (fluid_synth_t * synth, void *sfDataP, U32 sfDataLen, int reset_presets) {
    ...
    sfont = fluid_defsfloader_load(NULL, sfDataP, sfDataLen);
		if (sfont == NULL)
			return -1;
...
int
fluid_synth_sfunload (fluid_synth_t * synth, U32 id,
											int reset_presets) {
...
int fluid_synth_sfreload (fluid_synth_t * synth, void *sfDataP, U32 sfDataLen, U32 id) {
	fluid_sfont_t *sfont;
	int index = 0;
	fluid_list_t *list;
	fluid_sfloader_t *loader;
 ...
 */
fluid_sfont_t *fluid_synth_get_sfont (fluid_synth_t * synth, U32 num) {
	return (fluid_sfont_t *)
		fluid_list_get (fluid_list_nth (synth->sfont, num));
}
 ...
 */
void
fluid_synth_release_voice_on_same_note (fluid_synth_t * synth, int chan,
																				int key) {
	int i;
	fluid_voice_t *voice;
...
static fluid_tuning_t *fluid_synth_create_tuning (fluid_synth_t * synth,
																									int bank, int prog,
																									const char *name) {
	if ((bank < 0) || (bank >= 128)) {
		FLUID_LOG (FLUID_WARN, "Bank number out of range");
...
int
fluid_synth_select_tuning (fluid_synth_t * synth, int chan,
													 int bank, int prog) {
	fluid_tuning_t *tuning;
...
int
fluid_synth_start (fluid_synth_t * synth, U32 id,
...
int fluid_synth_stop (fluid_synth_t * synth, U32 id) {
	int i;
	fluid_voice_t *voice;
	int status = FLUID_FAILED;
	int count = 0;
...
int
fluid_synth_set_bank_offset (fluid_synth_t * synth, int sfont_id,
														 int offset) {
	fluid_bank_offset_t *bank_offset;
