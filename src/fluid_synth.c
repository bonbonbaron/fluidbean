...
static int fluid_synth_noteon_LOCAL (fluid_synth_t * synth, int chan, int key,
																		 int vel);
static int fluid_synth_noteoff_LOCAL (fluid_synth_t * synth, int chan,
																			int key);
static int fluid_synth_cc_LOCAL (fluid_synth_t * synth, int channum, int num);
static int fluid_synth_sysex_midi_tuning (fluid_synth_t * synth,
...
static int fluid_synth_modulate_voices_LOCAL (fluid_synth_t * synth, int chan,
																							int is_cc, int ctrl);
static int fluid_synth_modulate_voices_all_LOCAL (fluid_synth_t * synth,
																									int chan);
static int fluid_synth_update_channel_pressure_LOCAL (fluid_synth_t * synth,
																											int channum);
static int fluid_synth_update_key_pressure_LOCAL (fluid_synth_t * synth,
																									int chan, int key);
static int fluid_synth_update_pitch_bend_LOCAL (fluid_synth_t * synth,
																								int chan);
static int fluid_synth_update_pitch_wheel_sens_LOCAL (fluid_synth_t * synth,
																											int chan);
...
static fluid_preset_t *fluid_synth_get_preset (fluid_synth_t * synth,
																							 int sfontnum, int banknum,
																							 int prognum);
static fluid_preset_t *fluid_synth_get_preset_by_sfont_name (fluid_synth_t *
																														 synth,
																														 const char
																														 *sfontname,
																														 int banknum,
...
static fluid_voice_t *fluid_synth_free_voice_by_kill_LOCAL (fluid_synth_t *
																														synth);
...
static int fluid_synth_set_important_channels (fluid_synth_t * synth,
																							 const char *channels);


/* Callback handlers for real-time settings */
static void fluid_synth_handle_gain (void *data, const char *name,
																		 double value);
static void fluid_synth_handle_polyphony (void *data, const char *name,
																					int value);
static void fluid_synth_handle_device_id (void *data, const char *name,
																					int value);
static void fluid_synth_handle_overflow (void *data, const char *name,
																				 double value);
static void fluid_synth_handle_important_channels (void *data,
																									 const char *name,
																									 const char *value);
static void fluid_synth_handle_reverb_chorus_num (void *data,
																									const char *name,
																									double value);
static void fluid_synth_handle_reverb_chorus_int (void *data,
																									const char *name,
																									int value);


static void fluid_synth_reset_basic_channel_LOCAL (fluid_synth_t * synth,
																									 int chan, int nbr_chan);
static int fluid_synth_check_next_basic_channel (fluid_synth_t * synth,
																								 int basicchan, int mode,
																								 int val);
static void fluid_synth_set_basic_channel_LOCAL (fluid_synth_t * synth,
																								 int basicchan, int mode,
																								 int val);
...
static fluid_mod_t default_vel2att_mod;	/* SF2.01 section 8.4.1  */
																										/*not static */ fluid_mod_t default_vel2filter_mod;
																										/* SF2.01 section 8.4.2  */
...
void fluid_synth_settings (fluid_settings_t * settings) {
	...
	fluid_settings_register_num (settings, "synth.overflow.percussion", 4000,
															 -10000, 10000, 0);
	fluid_settings_register_num (settings, "synth.overflow.sustained", -1000,
															 -10000, 10000, 0);
	fluid_settings_register_num (settings, "synth.overflow.released", -2000,
															 -10000, 10000, 0);
	fluid_settings_register_num (settings, "synth.overflow.age", 1000, -10000,
															 10000, 0);
	fluid_settings_register_num (settings, "synth.overflow.volume", 500, -10000,
															 10000, 0);
	fluid_settings_register_num (settings, "synth.overflow.important", 5000,
															 -50000, 50000, 0);
	fluid_settings_register_str (settings, "synth.overflow.important-channels",
															 "", 0);
	...
	fluid_settings_register_int (settings, "synth.dynamic-sample-loading", 0, 0,
															 1, FLUID_HINT_TOGGLED);
}
...
static FLUID_INLINE void fluid_synth_add_ticks (fluid_synth_t * synth,
																								int val) {
...
static void fluid_sample_timer_process (fluid_synth_t * synth) {
	...
	for (st = synth->sample_timers; st; st = st->next) {
		...
		msec =
			(long) (1000.0 * ((double) (ticks - st->starttick)) /
							synth->sample_rate);
...
void delete_fluid_sample_timer (fluid_synth_t * synth,
																fluid_sample_timer_t * timer) {
...
void fluid_sample_timer_reset (fluid_synth_t * synth,
															 fluid_sample_timer_t * timer) {
...
fluid_synth_update_mixer (fluid_synth_t * synth,
													fluid_rvoice_function_t method, int intparam,
...
static FLUID_INLINE unsigned int
fluid_synth_get_min_note_length_LOCAL (fluid_synth_t * synth) {
	int i;
...
fluid_synth_t *new_fluid_synth (fluid_settings_t * settings) {
	...
	fluid_settings_getint (settings, "synth.reverb.active",
												 &synth->with_reverb);
	fluid_settings_getint (settings, "synth.chorus.active",
												 &synth->with_chorus);
	...
	fluid_settings_getnum_range (settings, "synth.sample-rate",
	...
	fluid_settings_getint (settings, "synth.effects-groups",
	...
	fluid_settings_getnum_float (settings, "synth.overflow.percussion",
															 &synth->overflow.percussion);
	fluid_settings_getnum_float (settings, "synth.overflow.released",
															 &synth->overflow.released);
	fluid_settings_getnum_float (settings, "synth.overflow.sustained",
															 &synth->overflow.sustained);
	fluid_settings_getnum_float (settings, "synth.overflow.volume",
															 &synth->overflow.volume);
	fluid_settings_getnum_float (settings, "synth.overflow.age",
															 &synth->overflow.age);
	fluid_settings_getnum_float (settings, "synth.overflow.important",
	...
	if (synth->audio_channels > synth->audio_groups) {
		...
		fluid_settings_setint (settings, "synth.audio-channels",
													 synth->audio_channels);
		FLUID_LOG (FLUID_WARN,
							 "Requested audio-channels to high. "
	...
	if (fluid_settings_dupstr (settings, "synth.overflow.important-channels",
		...
		if (fluid_synth_set_important_channels (synth, important_channels) !=
				FLUID_OK) {
	...
	fluid_synth_add_default_mod (synth, &default_vel2filter_mod,
															 FLUID_SYNTH_ADD);
	fluid_synth_add_default_mod (synth, &default_at2viblfo_mod,
															 FLUID_SYNTH_ADD);
	fluid_synth_add_default_mod (synth, &default_mod2viblfo_mod,
															 FLUID_SYNTH_ADD);
	...
	fluid_synth_add_default_mod (synth, &default_pitch_bend_mod,
															 FLUID_SYNTH_ADD);
	...
	if (with_ladspa) {
		...
		fluid_rvoice_mixer_set_ladspa (synth->eventhandler->mixer,
																	 synth->ladspa_fx, synth->audio_groups);
#else	/* LADSPA */
		FLUID_LOG (FLUID_WARN,
							 "FluidSynth has not been compiled with LADSPA support");
#endif /* LADSPA */
	}
	...
	FLUID_MEMSET (synth->channel, 0,
								synth->midi_channels * sizeof (*synth->channel));
	...
	for (i = 0; i < synth->nvoice; i++) {
		synth->voice[i] =
			new_fluid_voice (synth->eventhandler, synth->sample_rate);
	...
	fluid_synth_set_basic_channel_LOCAL (synth, 0,
...

	{
		...
		fluid_settings_getnum (settings, "synth.reverb.room-size",
													 &values[FLUID_REVERB_ROOMSIZE]);
		fluid_settings_getnum (settings, "synth.reverb.damp",
													 &values[FLUID_REVERB_DAMP]);
		fluid_settings_getnum (settings, "synth.reverb.width",
													 &values[FLUID_REVERB_WIDTH]);
		fluid_settings_getnum (settings, "synth.reverb.level",
...

	{
		...
		fluid_settings_getnum (settings, "synth.chorus.level",
													 &values[FLUID_CHORUS_LEVEL]);
		fluid_settings_getnum (settings, "synth.chorus.speed",
													 &values[FLUID_CHORUS_SPEED]);
		fluid_settings_getnum (settings, "synth.chorus.depth",
	...
	if (fluid_settings_str_equal (settings, "synth.midi-bank-select", "gm")) {
		...
		if (fluid_settings_str_equal (settings, "synth.midi-bank-select", "gs")) {
		synth->bank_select = FLUID_BANK_STYLE_GS;
	} else
		if (fluid_settings_str_equal (settings, "synth.midi-bank-select", "xg")) {
		synth->bank_select = FLUID_BANK_STYLE_XG;
	} else
		if (fluid_settings_str_equal (settings, "synth.midi-bank-select", "mma"))
	{
...
void delete_fluid_synth (fluid_synth_t * synth) {
	...
	fluid_settings_callback_num (synth->settings, "synth.gain", NULL, NULL);
	...
	fluid_settings_callback_str (synth->settings,
															 "synth.overflow.important-channels", NULL,
															 NULL);
	fluid_settings_callback_num (synth->settings, "synth.reverb.room-size",
															 NULL, NULL);
	fluid_settings_callback_num (synth->settings, "synth.reverb.damp", NULL,
															 NULL);
	fluid_settings_callback_num (synth->settings, "synth.reverb.width", NULL,
															 NULL);
	fluid_settings_callback_num (synth->settings, "synth.reverb.level", NULL,
															 NULL);
	fluid_settings_callback_int (synth->settings, "synth.reverb.active", NULL,
															 NULL);
	fluid_settings_callback_int (synth->settings, "synth.chorus.active", NULL,
															 NULL);
	fluid_settings_callback_int (synth->settings, "synth.chorus.nr", NULL,
															 NULL);
	fluid_settings_callback_num (synth->settings, "synth.chorus.level", NULL,
															 NULL);
	fluid_settings_callback_num (synth->settings, "synth.chorus.depth", NULL,
															 NULL);
	fluid_settings_callback_num (synth->settings, "synth.chorus.speed", NULL,
															 NULL);
	...
	for (list = synth->fonts_to_be_unloaded; list;
...
fluid_synth_noteon_LOCAL (fluid_synth_t * synth, int chan, int key, int vel) {
	...
	if (channel->preset == NULL) {
		if (synth->verbose) {
			FLUID_LOG (FLUID_INFO,
								 "noteon\t%d\t%d\t%d\t%05d\t%.3f\t%.3f\t%.3f\t%d\t%s", chan,
								 key, vel, 0, fluid_synth_get_ticks (synth) / 44100.0f,
	...
	if (fluid_channel_is_playing_mono (channel)) {	/* channel is mono or legato CC is On) */
		...
		/* a noteon poly is passed to fluid_synth_noteon_monopoly_legato().
		   ...
		   for details).
		 */
		return fluid_synth_noteon_monopoly_legato (synth, chan, INVALID_NOTE, key,
																							 vel);
	}
}
 ...
 */
int
fluid_synth_add_default_mod (fluid_synth_t * synth, const fluid_mod_t * mod,
														 int mode) {
	...
	fluid_return_val_if_fail ((mode == FLUID_SYNTH_ADD)
														|| (mode == FLUID_SYNTH_OVERWRITE), FLUID_FAILED);
 ...
 */
int
fluid_synth_remove_default_mod (fluid_synth_t * synth,
...
int fluid_synth_cc (fluid_synth_t * synth, int chan, int num, int val) {
	...
	if (channel->mode & FLUID_CHANNEL_ENABLED) {
		...
		if ((channel->mode & FLUID_CHANNEL_BASIC) &&
				((channel->mode & FLUID_CHANNEL_MODE_MASK) ==
...
static int fluid_synth_cc_LOCAL (fluid_synth_t * synth, int channum, int num) {
	...
	case OMNI_ON:
		...
		if (chan->mode & FLUID_CHANNEL_BASIC) {
			...
			value =
				fluid_synth_check_next_basic_channel (synth, channum, new_mode,
																							value);

			if (value != FLUID_FAILED) {
				/* reset the current basic channel before changing it */
				fluid_synth_reset_basic_channel_LOCAL (synth, channum,
																							 chan->mode_val);
	...
	case DATA_ENTRY_MSB:					/* not allowed to modulate (spec SF 2.01 - 8.2.1) */
		{
			...
			if (chan->nrpn_active) {	/* NRPN is active? */
				...
				case RPN_PITCH_BEND_RANGE:	/* Set bend range in semitones */
					fluid_channel_set_pitch_wheel_sensitivity (synth->channel[channum],
																										 value);
...
fluid_synth_sysex (fluid_synth_t * synth, const char *data, int len,
	...
	if (data[0] == MIDI_SYSEX_MANUF_ROLAND
			&& (data[1] == synth->device_id || data[1] == MIDI_SYSEX_DEVICE_ID_ALL)
			&& data[2] == MIDI_SYSEX_GS_ID && data[3] == MIDI_SYSEX_GS_DT1) {
		int result;
		fluid_synth_api_enter (synth);
...
static int
fluid_synth_sysex_midi_tuning (fluid_synth_t * synth, const char *data,
															 int len, char *response, int *response_len,
...
fluid_synth_sysex_gs_dt1 (fluid_synth_t * synth, const char *data, int len,
	...
	if (checksum != data[len - 1]) {
		FLUID_LOG (FLUID_INFO,
							 "SysEx DT1: dropping message on addr 0x%x due to incorrect checksum 0x%x. Correct checksum: 0x%x",
							 addr, (int) data[len - 1], checksum);
		return FLUID_FAILED;
	}
	...
	if ((addr & 0xFFF0FF) == 0x401015)	// Use for rhythm part
	{
		if (len_data > 1 || data[7] > 0x02) {
			FLUID_LOG (FLUID_INFO,
								 "SysEx DT1: dropping invalid rhythm part message");
			return FLUID_FAILED;
		}
		if (handled) {
			*handled = TRUE;
		...
		if (!dryrun) {
			...
			FLUID_LOG (FLUID_DBG, "SysEx DT1: setting MIDI channel %d to type %d",
								 chan, (int) synth->channel[chan]->channel_type);
...
int fluid_synth_all_notes_off_LOCAL (fluid_synth_t * synth, int chan) {
	...
	for (i = 0; i < synth->polyphony; i++) {
		...
		if (fluid_voice_is_playing (voice)
				&& ((-1 == chan) || (chan == fluid_voice_get_channel (voice)))) {
 ...
 */
static int
fluid_synth_modulate_voices_LOCAL (fluid_synth_t * synth, int chan, int is_cc,
																	 int ctrl) {
	fluid_voice_t *voice;
	int i;
...
fluid_synth_update_channel_pressure_LOCAL (fluid_synth_t * synth, int chan) {
	return fluid_synth_modulate_voices_LOCAL (synth, chan, 0,
...
static int
fluid_synth_update_key_pressure_LOCAL (fluid_synth_t * synth, int chan,
																			 int key) {
	fluid_voice_t *voice;
	int i;
	int result = FLUID_OK;
 ...
 */
static int
fluid_synth_set_preset (fluid_synth_t * synth, int chan,
												fluid_preset_t * preset) {
	fluid_channel_t *channel;

	fluid_return_val_if_fail (synth != NULL, FLUID_FAILED);
	fluid_return_val_if_fail (chan >= 0
														&& chan < synth->midi_channels, FLUID_FAILED);
...
static fluid_preset_t *fluid_synth_get_preset (fluid_synth_t * synth,
	...
	for (list = synth->sfont; list; list = fluid_list_next (list)) {
		...
		if (fluid_sfont_get_id (sfont) == sfontnum) {
			return fluid_sfont_get_preset (sfont, banknum - sfont->bankofs,
																		 prognum);
		}
	}
 ...
 * Returns preset pointer or NULL.
 */
fluid_preset_t *fluid_synth_find_preset (fluid_synth_t * synth, int banknum,
																				 int prognum) {
	fluid_preset_t *preset;
	fluid_sfont_t *sfont;
	fluid_list_t *list;
	...
	for (list = synth->sfont; list; list = fluid_list_next (list)) {
		...
		preset =
			fluid_sfont_get_preset (sfont, banknum - sfont->bankofs, prognum);
...
int fluid_synth_program_change (fluid_synth_t * synth, int chan, int prognum) {
	...
	if (prognum != FLUID_UNSET_PROGRAM) {
		...
		if (!preset) {
			...
			if (preset) {
				...
				FLUID_LOG (FLUID_WARN,
									 "No preset found on channel %d [bank=%d prog=%d]", chan,
									 banknum, prognum);
			}
		}
	}
	...
	fluid_channel_set_sfont_bank_prog (channel,
																		 preset ? fluid_sfont_get_id (preset->
																																	sfont) : 0,
																		 -1, prognum);
...
int fluid_synth_unset_program (fluid_synth_t * synth, int chan) {
	FLUID_API_ENTRY_CHAN (FLUID_FAILED);
	FLUID_API_RETURN (fluid_synth_program_change
										(synth, chan, FLUID_UNSET_PROGRAM));
}
 ...
 */
int
fluid_synth_pin_preset (fluid_synth_t * synth, int sfont_id, int bank_num,
												int preset_num) {
	int ret;
	fluid_preset_t *preset;
 ...
 */
int
fluid_synth_unpin_preset (fluid_synth_t * synth, int sfont_id, int bank_num,
													int preset_num) {
	int ret;
	fluid_preset_t *preset;
...
void
fluid_synth_set_sample_rate_immediately (fluid_synth_t * synth,
																				 float sample_rate) {
...
int fluid_synth_set_polyphony (fluid_synth_t * synth, int polyphony) {
	int result;
	fluid_return_val_if_fail (synth != NULL, FLUID_FAILED);
	fluid_return_val_if_fail (polyphony >= 1
														&& polyphony <= 65535, FLUID_FAILED);
	fluid_synth_api_enter (synth);
...
fluid_synth_update_polyphony_LOCAL (fluid_synth_t * synth, int new_polyphony) 
{
	...
	if (new_polyphony > synth->nvoice) {
		...
		for (i = synth->nvoice; i < new_polyphony; i++) {
			...
			fluid_voice_set_custom_filter (synth->voice[i],
																		 synth->custom_filter_type,
																		 synth->custom_filter_flags);
		}
...
fluid_synth_nwrite_float (fluid_synth_t * synth, int len,
	...
	if (synth->cur < FLUID_BUFSIZE) {
		...
		for (i = 0; i < synth->audio_channels; i++) {
#ifdef WITH_FLOAT
			FLUID_MEMCPY (left[i],
										&left_in[i * FLUID_BUFSIZE *
														 FLUID_MIXER_MAX_BUFFERS_DEFAULT + synth->cur],
										bytes);
			FLUID_MEMCPY (right[i],
										&right_in[i * FLUID_BUFSIZE *
															FLUID_MIXER_MAX_BUFFERS_DEFAULT + synth->cur],
										bytes);
#else	//WITH_FLOAT
			int j;
			...
			for (j = 0; j < num; j++) {
				left[i][j] =
					(float) left_in[i * FLUID_BUFSIZE *
													FLUID_MIXER_MAX_BUFFERS_DEFAULT + j + synth->cur];
				right[i][j] =
					(float) right_in[i * FLUID_BUFSIZE *
													 FLUID_MIXER_MAX_BUFFERS_DEFAULT + j + synth->cur];
			}
		...
		for (i = 0; i < synth->effects_channels; i++) {
			...
			if (fx_left != NULL) {
				FLUID_MEMCPY (fx_left[i],
											&fx_left_in[i * FLUID_BUFSIZE *
																	FLUID_MIXER_MAX_BUFFERS_DEFAULT +
																	synth->cur], bytes);
			}

			if (fx_right != NULL) {
				FLUID_MEMCPY (fx_right[i],
											&fx_right_in[i * FLUID_BUFSIZE *
																	 FLUID_MIXER_MAX_BUFFERS_DEFAULT +
																	 synth->cur], bytes);
			}
			...
			if (fx_left != NULL) {
				for (j = 0; j < num; j++) {
					fx_left[i][j] =
						(float) fx_left_in[i * FLUID_BUFSIZE *
															 FLUID_MIXER_MAX_BUFFERS_DEFAULT + j +
															 synth->cur];
				}
			}
			...
			if (fx_right != NULL) {
				for (j = 0; j < num; j++) {
					fx_right[i][j] =
						(float) fx_right_in[i * FLUID_BUFSIZE *
																FLUID_MIXER_MAX_BUFFERS_DEFAULT + j +
																synth->cur];
				}
			}
	...
	while (count < len) {
		...
		for (i = 0; i < synth->audio_channels; i++) {
#ifdef WITH_FLOAT
			FLUID_MEMCPY (left[i] + count,
										&left_in[i * FLUID_BUFSIZE *
														 FLUID_MIXER_MAX_BUFFERS_DEFAULT], bytes);
			FLUID_MEMCPY (right[i] + count,
										&right_in[i * FLUID_BUFSIZE *
															FLUID_MIXER_MAX_BUFFERS_DEFAULT], bytes);
#else	//WITH_FLOAT
			int j;
		...
		for (i = 0; i < synth->effects_channels; i++) {
			...
			if (fx_left != NULL) {
				FLUID_MEMCPY (fx_left[i] + count,
											&fx_left_in[i * FLUID_BUFSIZE *
																	FLUID_MIXER_MAX_BUFFERS_DEFAULT], bytes);
			}

			if (fx_right != NULL) {
				FLUID_MEMCPY (fx_right[i] + count,
											&fx_right_in[i * FLUID_BUFSIZE *
																	 FLUID_MIXER_MAX_BUFFERS_DEFAULT], bytes);
			}
 ...
 */
static FLUID_INLINE void fluid_synth_mix_single_buffer (float *FLUID_RESTRICT
																												out, int ooff,
																												const fluid_real_t *
																												FLUID_RESTRICT in,
																												int ioff, int buf_idx,
																												int num) {
	if (out != NULL) {
		int j;
		...
		for (j = 0; j < num; j++) {
			out[j + ooff] +=
				(float) in[buf_idx * FLUID_BUFSIZE * FLUID_MIXER_MAX_BUFFERS_DEFAULT +
									 j + ioff];
		}
	}
}
...
int fluid_synth_process_LOCAL (fluid_synth_t * synth, int len, int nfx,
	...
	fluid_return_val_if_fail (0 <= nfx / 2
														&& nfx / 2 <= nfxchan * nfxunits, FLUID_FAILED);
	fluid_return_val_if_fail (0 <= nout / 2
														&& nout / 2 <= naudchan, FLUID_FAILED);
	...
	if (synth->cur < buffered_blocks * FLUID_BUFSIZE) {
		...
		if (nout != 0) {
			for (i = 0; i < naudchan; i++) {
				...
				fluid_synth_mix_single_buffer (out_buf, 0, left_in, synth->cur, i,
																			 num);

				/* mix num right samples from input mixer buffer (right_in) at input offset
				   synth->cur to output buffer (out_buf) at offset 0 */
				out_buf = out[(i * 2 + 1) % nout];
				fluid_synth_mix_single_buffer (out_buf, 0, right_in, synth->cur, i,
																			 num);
			}
		}
		...
		if (nfx != 0) {
			...
			for (f = 0; f < nfxunits; f++) {
				...
				for (i = 0; i < nfxchan; i++) {
					...
					fluid_synth_mix_single_buffer (out_buf, 0, fx_left_in, synth->cur,
																				 buf_idx, num);

					/* mix num right samples from input mixer buffer (fx_right_in) at input offset
					   synth->cur to output buffer (out_buf) at offset 0 */
					out_buf = fx[(buf_idx * 2 + 1) % nfx];
					fluid_synth_mix_single_buffer (out_buf, 0, fx_right_in, synth->cur,
																				 buf_idx, num);
				}
			}
		}
			...
			for (f = 0; f < nfxunits; f++) {
				...
				for (i = 0; i < nfxchan; i++) {
					...
					/* mix num left samples from input mixer buffer (fx_left_in) at input offset
					   0 to output buffer (out_buf) at offset count */
					float *out_buf = fx[(buf_idx * 2) % nfx];
					fluid_synth_mix_single_buffer (out_buf, count, fx_left_in, 0,
																				 buf_idx, num);

					/* mix num right samples from input mixer buffer (fx_right_in) at input offset
					   0 to output buffer (out_buf) at offset count */
					out_buf = fx[(buf_idx * 2 + 1) % nfx];
					fluid_synth_mix_single_buffer (out_buf, count, fx_right_in, 0,
																				 buf_idx, num);
				}
			}
		}
...
fluid_synth_write_float_channels_LOCAL (fluid_synth_t * synth, int len,
	...
	fluid_return_val_if_fail (bufs_in_count <= synth->audio_groups,
														FLUID_FAILED);
	...
	do {
		...
		do {
			...
			do {
				...
				int in_idx =
					--i * FLUID_BUFSIZE * FLUID_MIXER_MAX_BUFFERS_DEFAULT + n;
	...
	fluid_profile_write (FLUID_PROF_WRITE, prof_ref,
											 fluid_rvoice_mixer_get_active_voices (synth->
																														 eventhandler->
																														 mixer), len);
	return FLUID_OK;
}
 ...
 * 16 bit audio.
 */
int fluid_synth_write_s16_channels (fluid_synth_t * synth, int len, int channels_count, void *channels_out[], int channels_off[], int channels_incr[]) {
	...
	do {
		...
		if (cur >= synth->curmax) {
			...
			int blocksleft = (size + FLUID_BUFSIZE - 1) / FLUID_BUFSIZE;
			synth->curmax =
				FLUID_BUFSIZE * fluid_synth_render_blocks (synth, blocksleft);
  ...
  // MB erase this crap later after proving this works
 ...
 */
void
fluid_synth_dither_s16 (int *dither_index, int len, const float *lin,
												const float *rin, void *lout, int loff, int lincr,
												void *rout, int roff, int rincr) {
	int i, j, k;
...
static void fluid_synth_check_finished_voices (fluid_synth_t * synth) {
	int j;
	fluid_rvoice_t *fv;

	while (NULL !=
				 (fv =
					fluid_rvoice_eventhandler_get_finished_voice (synth->
																												eventhandler))) {
...
//  synth->synth_thread_id = fluid_thread_get_id ();
	...
	blockcount =
		fluid_rvoice_mixer_render (synth->eventhandler->mixer, blockcount);
	...
	fluid_profile (FLUID_PROF_ONE_BLOCK, prof_ref,
								 fluid_rvoice_mixer_get_active_voices (synth->eventhandler->
																											 mixer),
								 blockcount * FLUID_BUFSIZE);
	return blockcount;
}
...
static fluid_voice_t *fluid_synth_free_voice_by_kill_LOCAL (fluid_synth_t *
	...
	FLUID_LOG (FLUID_DBG, "Killing voice %d, index %d, chan %d, key %d ",
						 fluid_voice_get_id (voice), best_voice_index,
...
fluid_voice_t *fluid_synth_alloc_voice (fluid_synth_t * synth,
	...
	FLUID_API_RETURN (fluid_synth_alloc_voice_LOCAL
										(synth, sample, chan, key, vel, NULL));

}

fluid_voice_t *fluid_synth_alloc_voice_LOCAL (fluid_synth_t * synth,
	 ...
	 */
	{
		...
		while (default_mod != NULL) {
			if (
				...
				fluid_voice_add_mod_local (voice, &custom_breath2att_mod,
																	 FLUID_VOICE_DEFAULT, 0);
			} else {
...
fluid_synth_kill_by_exclusive_class_LOCAL (fluid_synth_t * synth,
	...
	for (i = 0; i < synth->polyphony; i++) {
		...
		if (fluid_voice_is_playing (existing_voice)
				...
				&& fluid_voice_gen_value (existing_voice,
																	GEN_EXCLUSIVECLASS) == excl_class
 ...
 */
int
fluid_synth_sfload (fluid_synth_t * synth, const char *filename,
										int reset_presets) {
	fluid_sfont_t *sfont;
	fluid_list_t *list;
	fluid_sfloader_t *loader;
  ...
  printf("4\n");
    ...
    // List is the same as the list nodes we used at FANUC.
		for (list = synth->loaders; list; list = fluid_list_next (list)) {
			loader = (fluid_sfloader_t *) fluid_list_get (list);
      printf("about to call sfloader load...\n");
			sfont = fluid_sfloader_load (loader, filename);
      printf("just got out of it\n");
			if (sfont != NULL) {
				sfont->refcount++;
...
int fluid_synth_sfreload (fluid_synth_t * synth, int id) {
	...
	for (list = synth->sfont, index = 0; list;
			 list = fluid_list_next (list), index++) {
		sfont = fluid_list_get (list);
 ...
 */
void
fluid_synth_get_voicelist (fluid_synth_t * synth, fluid_voice_t * buf[],
													 int bufsize, int id) {
	int count = 0;
	int i;
...
int fluid_synth_reverb_on (fluid_synth_t * synth, int fx_group, int on) {
	...
	ret = fluid_rvoice_eventhandler_push (synth->eventhandler,
																				...
																				synth->eventhandler->mixer, param);
...
int fluid_synth_set_reverb_preset (fluid_synth_t * synth, unsigned int num) {
	...
	fluid_return_val_if_fail (num < FLUID_N_ELEMENTS (revmodel_preset),
														FLUID_FAILED);
 ...
 */
int
fluid_synth_set_reverb (fluid_synth_t * synth, double roomsize,
	...
	return fluid_synth_set_reverb_full (synth, -1, FLUID_REVMODEL_SET_ALL,
																			values);
}
...
int fluid_synth_set_reverb_roomsize (fluid_synth_t * synth, double roomsize) {
	return fluid_synth_reverb_set_param (synth, -1, FLUID_REVERB_ROOMSIZE,
																			 roomsize);
}
...
int fluid_synth_set_reverb_group_roomsize (fluid_synth_t * synth,
																					 int fx_group, double roomsize) {
	return fluid_synth_reverb_set_param (synth, fx_group, FLUID_REVERB_ROOMSIZE,
																			 roomsize);
}
...
int fluid_synth_set_reverb_group_damp (fluid_synth_t * synth, int fx_group,
																			 double damping) {
	return fluid_synth_reverb_set_param (synth, fx_group, FLUID_REVERB_DAMP,
																			 damping);
}
...
int fluid_synth_set_reverb_group_width (fluid_synth_t * synth, int fx_group,
																				double width) {
	return fluid_synth_reverb_set_param (synth, fx_group, FLUID_REVERB_WIDTH,
																			 width);
}
...
int fluid_synth_set_reverb_group_level (fluid_synth_t * synth, int fx_group,
																				double level) {
	return fluid_synth_reverb_set_param (synth, fx_group, FLUID_REVERB_LEVEL,
																			 level);
}
...
fluid_synth_reverb_set_param (fluid_synth_t * synth, int fx_group,
	...
	/* check parameters */
	fluid_return_val_if_fail (synth != NULL, FLUID_FAILED);
	fluid_return_val_if_fail ((param >= 0)
														&& (param < FLUID_REVERB_PARAM_LAST),
														FLUID_FAILED);
	fluid_synth_api_enter (synth);
	...
	values[param] = value;
	ret =
		fluid_synth_set_reverb_full (synth, fx_group,
																 FLUID_REVPARAM_TO_SETFLAG (param), values);
	FLUID_API_RETURN (ret);
}
...
int fluid_synth_get_reverb_group_roomsize (fluid_synth_t * synth,
																					 int fx_group, double *roomsize) {
	return fluid_synth_reverb_get_param (synth, fx_group, FLUID_REVERB_ROOMSIZE,
																			 roomsize);
}
...
int fluid_synth_get_reverb_group_damp (fluid_synth_t * synth, int fx_group,
																			 double *damping) {
	return fluid_synth_reverb_get_param (synth, fx_group, FLUID_REVERB_DAMP,
																			 damping);
}
...
int fluid_synth_get_reverb_group_width (fluid_synth_t * synth, int fx_group,
																				double *width) {
	return fluid_synth_reverb_get_param (synth, fx_group, FLUID_REVERB_WIDTH,
																			 width);
}
...
int fluid_synth_get_reverb_group_level (fluid_synth_t * synth, int fx_group,
																				double *level) {
	return fluid_synth_reverb_get_param (synth, fx_group, FLUID_REVERB_LEVEL,
																			 level);
}
...
{
	return fluid_synth_chorus_set_param (synth, fx_group, FLUID_CHORUS_NR,
																			 (double) nr);
}
 ...
 */
int
fluid_synth_set_chorus_group_level (fluid_synth_t * synth, int fx_group,
																		double level) {
	return fluid_synth_chorus_set_param (synth, fx_group, FLUID_CHORUS_LEVEL,
																			 level);
}
 ...
 */
int
fluid_synth_set_chorus_group_speed (fluid_synth_t * synth, int fx_group,
																		double speed) {
	return fluid_synth_chorus_set_param (synth, fx_group, FLUID_CHORUS_SPEED,
																			 speed);
}
 ...
 */
int
fluid_synth_set_chorus_group_depth (fluid_synth_t * synth, int fx_group,
																		double depth_ms) {
	return fluid_synth_chorus_set_param (synth, fx_group, FLUID_CHORUS_DEPTH,
																			 depth_ms);
}
 ...
 */
int
fluid_synth_set_chorus_group_type (fluid_synth_t * synth, int fx_group,
																	 int type) {
	return fluid_synth_chorus_set_param (synth, fx_group, FLUID_CHORUS_TYPE,
																			 (double) type);
}
...
fluid_synth_chorus_set_param (fluid_synth_t * synth, int fx_group, int param,
	...
	ret = fluid_synth_set_chorus_full (synth, fx_group,
																		 FLUID_CHORPARAM_TO_SETFLAG (param),
																		 values);
	FLUID_API_RETURN (ret);
}
 ...
 */
int
fluid_synth_get_chorus_group_nr (fluid_synth_t * synth, int fx_group,
																 int *nr) {
	double num_nr = 0.0;
	int status;
	status =
		fluid_synth_chorus_get_param (synth, fx_group, FLUID_CHORUS_NR, &num_nr);
	*nr = (int) num_nr;
	return status;
}
 ...
 */
int
fluid_synth_get_chorus_group_level (fluid_synth_t * synth, int fx_group,
																		double *level) {
	return fluid_synth_chorus_get_param (synth, fx_group, FLUID_CHORUS_LEVEL,
																			 level);
}
 ...
 */
int
fluid_synth_get_chorus_group_speed (fluid_synth_t * synth, int fx_group,
																		double *speed) {
	return fluid_synth_chorus_get_param (synth, fx_group, FLUID_CHORUS_SPEED,
																			 speed);
}
 ...
 */
int
fluid_synth_get_chorus_group_depth (fluid_synth_t * synth, int fx_group,
																		double *depth_ms) {
	return fluid_synth_chorus_get_param (synth, fx_group, FLUID_CHORUS_DEPTH,
																			 depth_ms);
}
 ...
 */
int
fluid_synth_get_chorus_group_type (fluid_synth_t * synth, int fx_group,
																	 int *type) {
	double num_type = 0.0;
	int status;
	status =
		fluid_synth_chorus_get_param (synth, fx_group, FLUID_CHORUS_TYPE,
																	&num_type);
	*type = (int) num_type;
	return status;
}
...
static fluid_tuning_t *fluid_synth_get_tuning (fluid_synth_t * synth,
	...
	if ((synth->tuning == NULL) ||
			(synth->tuning[bank] == NULL) || (synth->tuning[bank][prog] == NULL)) {
		return NULL;
	}
...
static int
fluid_synth_replace_tuning_LOCK (fluid_synth_t * synth,
																 fluid_tuning_t * tuning, int bank, int prog,
																 int apply) {
	fluid_tuning_t *old_tuning;
	...
	if (old_tuning) {
		if (!fluid_tuning_unref (old_tuning, 1)) {	/* -- unref old tuning */
			...
			fluid_synth_replace_tuning_LOCAL (synth, old_tuning, tuning, apply,
																				FALSE);
		}
	}
...
static void
fluid_synth_replace_tuning_LOCAL (fluid_synth_t * synth,
																	fluid_tuning_t * old_tuning,
																	fluid_tuning_t * new_tuning, int apply,
																	int unref_new) {
	fluid_channel_t *channel;
...
static void
fluid_synth_update_voice_tuning_LOCAL (fluid_synth_t * synth,
																			 fluid_channel_t * channel) {
	fluid_voice_t *voice;
	int i;
...
fluid_synth_activate_key_tuning (fluid_synth_t * synth, int bank, int prog,
	...
	if (tuning) {
		...
		retval =
			fluid_synth_replace_tuning_LOCK (synth, tuning, bank, prog, apply);
...
fluid_synth_tune_notes (fluid_synth_t * synth, int bank, int prog,
												int len, const int *key, const double *pitch,
												int apply) {
	...
	if (new_tuning) {
		...
		retval =
			fluid_synth_replace_tuning_LOCK (synth, new_tuning, bank, prog, apply);
 ...
 */
int
fluid_synth_activate_tuning (fluid_synth_t * synth, int chan, int bank,
														 int prog, int apply) {
	fluid_tuning_t *tuning;
	int retval = FLUID_OK;
 ...
 */
int
fluid_synth_tuning_iteration_next (fluid_synth_t * synth, int *bank,
																	 int *prog) {
	void *pval;
	int b = 0, p = 0;
...
fluid_synth_tuning_dump (fluid_synth_t * synth, int bank, int prog,
	...
	if (tuning) {
		...
		if (pitch) {
			FLUID_MEMCPY (pitch, fluid_tuning_get_all (tuning),
										128 * sizeof (double));
		}
	}
...
static void
fluid_synth_set_gen_LOCAL (fluid_synth_t * synth, int chan, int param,
													 float value) {
	fluid_voice_t *voice;
	int i;
...
int fluid_synth_handle_midi_event (void *data, fluid_midi_event_t * event) {
	...
	case NOTE_OFF:
		return fluid_synth_noteoff (synth, chan,
																fluid_midi_event_get_key (event));
	...
	case MIDI_SYSEX:
		return fluid_synth_sysex (synth, event->paramptr, event->param1, NULL,
															NULL, NULL, FALSE);
 ...
 */
int
fluid_synth_start (fluid_synth_t * synth, unsigned int id,
									 fluid_preset_t * preset, int audio_chan, int chan, int key,
									 int vel) {
	...
	if (dynamic_samples) {
		...
		FLUID_LOG (FLUID_ERR,
							 "Calling fluid_synth_start() while synth.dynamic-sample-loading is enabled is not supported.");
...
int fluid_synth_set_channel_type (fluid_synth_t * synth, int chan, int type) {
	fluid_return_val_if_fail ((type >= CHANNEL_TYPE_MELODIC)
														&& (type <= CHANNEL_TYPE_DRUM), FLUID_FAILED);
	FLUID_API_ENTRY_CHAN (FLUID_FAILED);
...
int fluid_synth_set_custom_filter (fluid_synth_t * synth, int type, int flags) {
	...
	fluid_return_val_if_fail (synth != NULL, FLUID_FAILED);
	fluid_return_val_if_fail (type >= FLUID_IIR_DISABLED
														&& type < FLUID_IIR_LAST, FLUID_FAILED);
...
static int fluid_synth_set_important_channels (fluid_synth_t * synth,
	...
	FLUID_MEMSET (scores->important_channels, FALSE,
								sizeof (*scores->important_channels) *
								scores->num_important_channels);
	...
	if (channels != NULL) {
		 ...
		 * 1-based. */
		num_values =
			fluid_settings_split_csv (channels, values, synth->midi_channels);
 ...
 */
int fluid_synth_set_legato_mode (fluid_synth_t * synth, int chan,
																 int legatomode) {
	/* checks parameters first */
	fluid_return_val_if_fail (legatomode >= 0, FLUID_FAILED);
	fluid_return_val_if_fail (legatomode < FLUID_CHANNEL_LEGATO_MODE_LAST,
														FLUID_FAILED);
	FLUID_API_ENTRY_CHAN (FLUID_FAILED);
 ...
 */
int fluid_synth_get_legato_mode (fluid_synth_t * synth, int chan,
																 int *legatomode) {
	/* checks parameters first */
...
int fluid_synth_set_portamento_mode (fluid_synth_t * synth, int chan,
	...
	fluid_return_val_if_fail (portamentomode <
														FLUID_CHANNEL_PORTAMENTO_MODE_LAST, FLUID_FAILED);
	FLUID_API_ENTRY_CHAN (FLUID_FAILED);
 ...
 *   - \a chan is outside MIDI channel count.
 */
int fluid_synth_set_breath_mode (fluid_synth_t * synth, int chan,
																 int breathmode) {
	/* checks parameters first */
	FLUID_API_ENTRY_CHAN (FLUID_FAILED);
 ...
 */
int fluid_synth_get_breath_mode (fluid_synth_t * synth, int chan,
																 int *breathmode) {
	/* checks parameters first */
...
*/
static void
fluid_synth_reset_basic_channel_LOCAL (fluid_synth_t * synth, int chan,
																			 int nbr_chan) {
	int i;
 ...
 */
static int
fluid_synth_check_next_basic_channel (fluid_synth_t * synth, int basicchan,
																			int mode, int val) {
...
int fluid_synth_set_basic_channel (fluid_synth_t * synth, int chan, int mode,
	...
	if (val == FLUID_FAILED
			|| synth->channel[chan]->mode & FLUID_CHANNEL_ENABLED) {
...
*/
static void
fluid_synth_set_basic_channel_LOCAL (fluid_synth_t * synth, int basicchan,
																		 int mode, int val) {
	int i;
 ...
 */
static int fluid_synth_get_previous_basic_channel (fluid_synth_t * synth,
																									 int chan) {
	for (; chan >= 0; chan--) {
...
int fluid_synth_get_basic_channel (fluid_synth_t * synth, int chan,
	...
	if ((synth->channel[chan]->mode & FLUID_CHANNEL_ENABLED) &&
			...
			(basic_chan =
			 fluid_synth_get_previous_basic_channel (synth,
																							 chan)) != FLUID_FAILED) {
