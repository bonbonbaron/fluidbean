...
#endif
#define readU8_(srcP_, dstP_)   G_STMT_START {        \
...
static int load_body (Inflatable * infP, U8 **inPP, SFData * sf);
static int read_listchunk (SFChunk * chunk, U8 **srcPP);
static int process_info (U8 ** srcPP, int size, SFData * sf);
static int process_sdta (U8 ** srcPP, int size, SFData * sf);
static int pdtahelper (U8 ** srcPP, unsigned int expid, unsigned int reclen,
											 SFChunk * chunk, int *size);
static int process_pdta (U8 ** srcPP, int size, SFData * sf);
...
static int load_shdr (U8 ** srcPP, unsigned int size, SFData * sf);
...
int fluid_defsfont_load (fluid_defsfont_t * sfont, Inflatable *infP) {
	SFData *sfdata;
	fluid_list_t *p;
	SFPreset *sfpreset;
	SFSample *sfsample;
  ...
  U8 *inP = infP->inflatedDataP;
	...
	if (fluid_defsfont_load_sampledata (sfont, infP) != FLUID_OK)
		goto err_exit;
 ...
 */
int fluid_defsfont_load_sampledata(fluid_defsfont_t *sfont, Inflatable *infP) {
	unsigned short endian;
  // Put data at sample of interest.
  U8 *dataP = ((U8*) infP->inflatedDataP) + sfont->samplepos;
...
SFData *sfLoadMem (Inflatable * infP, U8 **inPP) {
	SFData *sfP = NULL;
	int err = FALSE;
	...
	if (!err && !load_body(infP, inPP, sfP))
...
static int load_body (Inflatable * infP, U8 **inPP, SFData * sfP) {
	SFChunk chunk;								// 8 bytes; 4 for id, 4 for size

  U8 *inP = *inPP;
	// Load RIFF chunk.
	readChunk_(&chunk, inP);
	if (chunk.id != RIFF_) return FAIL;

  // Sfbk (soundfont bank, I think?)
	readId_(&chunk.id, inP);
	if (chunk.id != sfbk_) return FAIL;
	if (chunk.size != infP->inflatedLen - 8) return FAIL;
	...
	if (!process_sdta(chunk.size, sf, infP, &inP)) return FAIL;
...
static int process_sdta (int size, SFData * sf, Inflatable *infP, U8 **inPP) {
	SFChunk chunk;
  ...
  U8 *inP = *inPP;
	...
	/* sample data follows */
	sf->samplepos = inP - (U8*) infP->inflatedDataP;

	/* used in fixup_sample() to check validity of sample headers */
	sdtachunk_size = chunk.size;
	sf->samplesize = chunk.size;

	skip_(inP, chunk.size);

	return OK;
}

static int pdtahelper (unsigned int expid, unsigned int reclen, SFChunk * chunk, int *size, U8 **inPP) {
	unsigned int id;
  U8 *inP = *inPP;
...
static int process_pdta (int size, SFData * sf, U8 **inPP) {
	SFChunk chunk;
  U8 *inP = *inPP;
...
static int load_phdr (int size, SFData * sf, U8 **inPP) {
	int i, i2;
  U8 *inP = *inPP;
...
/* preset bag loader */
static int load_pbag (int size, SFData * sf, U8 **inPP) {
	fluid_list_t *p, *p2;
	SFZone *z, *pz = NULL;
...
static int load_pmod (int size, SFData * sf, U8 **inPP) {
  ...
  U8 *inP = *inPP;
	...
	size -= SFMODSIZE;
	if (size != 0) return FAIL;
	skip_ (inP, SFMODSIZE);	/* terminal mod */
 ...
 *------------------------------------------------------------------- */
static int load_pgen (int size, SFData * sf, U8 **inPP) {
  ...
  U8 *inP = *inPP;
	...
	if (size != 0)
    return FAIL;
	skip_ (inP, SFGENSIZE);	/* terminal gen */

	return (OK);
}

/* instrument header loader */
static int load_ihdr (int size, SFData * sf, U8 **inPP) {
	int i, i2;
...
static int load_ibag (int size, SFData * sf, U8 **inPP) {
	fluid_list_t *p, *p2;
	SFZone *z, *pz = NULL;
...
static int load_imod (int size, SFData * sf, U8 **inPP) {
	fluid_list_t *p, *p2, *p3;
	SFMod *m;
...
static int load_igen (int size, SFData * sf, U8 **inPP) {
	fluid_list_t *p, *p2, *p3, *dup, **hz = NULL;
	SFZone *z;
	SFGen *g;
	SFGenAmount genval;
...
static int load_shdr (unsigned int size, SFData * sf, U8 **inPP) {
	unsigned int i;
	SFSample *p;
  ...
  U8 *inP = *inPP;
	...
	for (i = 0; i < size; i++) {
		...
		readStr_ (p->name, inP);
		readU32_ (p->start, inP);
		readU32_ (p->end, inP);		/* - end, loopstart and loopend */
	...
	skip_ (inP, SFSHDRSIZE);	/* skip terminal shdr */
