...
//removed inline
static void fluid_voice_effects (Voice * voice, int count,
 ...
 * delete_fluid_voice
 */
int delete_fluid_voice (Voice * voice) {
	if (voice == NULL) {
		return FLUID_OK;
	}
	FLUID_FREE (voice);
 ...
 */
int fluid_voice_init (Voice * voice, Sample * sample, Channel * channel, int key, int vel, U32 id, U32 start_time, fluid_real_t gain) {
...
void fluid_voice_gen_set (Voice * voice, int i, float val) {
	voice->gen[i].val = val;
	voice->gen[i].flags = GEN_SET;
}

void fluid_voice_gen_incr (Voice * voice, int i, float val) {
	voice->gen[i].val += val;
	voice->gen[i].flags = GEN_SET;
}

float fluid_voice_gen_get (Voice * voice, int gen) {
	return voice->gen[gen].val;
}

fluid_real_t fluid_voice_gen_value (Voice * voice, int num) {
 ...
 */
static void
fluid_voice_effects (Voice * voice, int count,
										 fluid_real_t * dsp_left_buf,
										 fluid_real_t * dsp_right_buf,
										 fluid_real_t * dsp_reverb_buf,
										 fluid_real_t * dsp_chorus_buf) {
	...
	if (dsp_filter_coeff_incr_count > 0) {
		...
		for (dsp_i = 0; dsp_i < count; dsp_i++) {	/* The filter is implemented in Direct-II form. */
			dsp_centernode =
				dsp_buf[dsp_i] - dsp_a1 * dsp_hist1 - dsp_a2 * dsp_hist2;
			dsp_buf[dsp_i] =
				dsp_b02 * (dsp_centernode + dsp_hist2) + dsp_b1 * dsp_hist1;
 ...
 */
Channel *fluid_voice_get_channel (Voice * voice) {
 ...
 * cents. */
int fluid_voice_calculate_runtime_synthesis_parameters (Voice * voice) {
	int i;
	...
	for (i = 0; i < voice->mod_count; i++) {
		...
		Generator *dest_gen = &voice->gen[dest_gen_index];
		dest_gen->mod += modval;
 ...
 */
int
calculate_hold_decay_buffers (Voice * voice, int gen_base,
 ...
 * of all three.
 */
void fluid_voice_update_param (Voice * voice, int gen) {
	double q_dB;
	fluid_real_t x;
	fluid_real_t y;
	U32 count;
	...
	case GEN_ATTENUATION:
		voice->attenuation =
			((fluid_real_t) (voice)->gen[GEN_ATTENUATION].val *
			 ALT_ATTENUATION_SCALE) +
			(fluid_real_t) (voice)->gen[GEN_ATTENUATION].mod +
			(fluid_real_t) (voice)->gen[GEN_ATTENUATION].nrpn;
	...
	case GEN_ENDADDRCOARSEOFS:		/* SF2.01 section 8.1.3 # 12 */
		if (voice->sample != NULL) {
			voice->end = (voice->sample->end + (int) _GEN (voice, GEN_ENDADDROFS)
 ...
 * */
int fluid_voice_modulate (Voice * voice, int cc, int ctrl) {
	int i, k;
	Modulator *mod;
	int gen;
	fluid_real_t modval;
 ...
 *
 */
int fluid_voice_modulate_all (Voice * voice) {
	Modulator *mod;
	int i, k, gen;
	fluid_real_t modval;
 ...
 * fluid_voice_noteoff
 */
int fluid_voice_noteoff (Voice * voice) {
	U32 at_tick;
 ...
 */
int fluid_voice_kill_excl (Voice * voice) {
 ...
 */
int fluid_voice_off (Voice * voice) {
	voice->chan = NO_CHANNEL;
 ...
 *                             Don't check.
 */
void fluid_voice_add_mod (Voice * voice, Modulator * mod, int mode) {
	int i;
	...
	if (mode == FLUID_VOICE_ADD) {
		...
		for (i = 0; i < voice->mod_count; i++) {
			if (Modulatorest_identity (&voice->mod[i], mod)) {
...
U32 fluid_voice_get_id (Voice * voice) {
	return voice->id;
}

int fluid_voice_is_playing (Voice * voice) {
 ...
 */
fluid_real_t
fluid_voice_get_lower_boundary_for_attenuation (Voice * voice) {
	int i;
	Modulator *mod;
...
int
fluid_voice_set_param (Voice * voice, int gen,
...
int fluid_voice_set_gain (Voice * voice, fluid_real_t gain) {
 ...
 * - Store in sample
 */
int fluid_voice_optimize_sample (Sample * s) {
